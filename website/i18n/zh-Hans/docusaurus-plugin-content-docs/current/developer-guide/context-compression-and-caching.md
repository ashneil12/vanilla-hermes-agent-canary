...
设置 `context.engine: "lossless"` 可启用内置 Hermes 原生 lossless 引擎。它会把原始消息保存到
`$HERMES_HOME/context_engines/lossless/lossless_context.db`，用紧凑恢复标记替换较早的 prompt 片段，并向模型暴露
`lcm_grep`、`lcm_describe`、`lcm_expand` 和 `lcm_status` 工具用于检索。该模式需要显式启用，不会改变默认 `compressor` 行为。
...
中间上下文压力警告已被移除（参见 `run_agent.py` 中的迭代预算块，其中注明："No intermediate pressure warnings — they caused models to 'give up' prematurely on complex tasks"）。压缩在 prompt token 达到配置的 `compression.threshold`（默认 85%）时触发，无需事先警告步骤；gateway 会话清理也作为二级安全网在模型上下文窗口的 85% 处触发。
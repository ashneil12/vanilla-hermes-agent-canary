...
context:
  ...
  engine: "lossless"      # 激活内置 lossless 上下文引擎
```
...
Hermes 在 `plugins/context_engine/lossless/` 提供 V0 lossless context engine。
它默认不启用。启用方式：
...
该引擎会在压缩前把原始消息持久化到
`$HERMES_HOME/context_engines/lossless/lossless_context.db`，再用紧凑的
恢复标记替换较早的 prompt 片段。启用 `context_engine` toolset 时，模型可使用：

- `lcm_grep` — 搜索已持久化的原始消息和摘要标记。
- `lcm_describe` — 查看 `msg_*` 或 `sum_*` 记录。
- `lcm_expand` — 恢复 `sum_*` 标记背后的源消息。
- `lcm_status` — 查看存储和会话计数。

这是 Hermes 原生实现，不是 OpenClaw 插件 shim。它直接实现
`ContextEngine` ABC，并遵循 Hermes 会话生命周期 hook。

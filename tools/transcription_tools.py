...
# hermes-fork: cloud STT auto-detect + graceful fallback (incl. Venice).
def _auto_detect_cloud_stt() -> str:
    ...
    """
    # Upstream (2026-07) routes these reads through the shared voice-key
    # resolver so a profile's secret scope wins over raw env. Ported here
    # because the fork keeps the chain in this helper rather than inline.
...
def _get_provider(stt_config: dict) -> str:
    ...
    *local* choice without a local backend resolves to ``none`` rather than a
    ...
    if explicit:
        ...
        if provider == "venice":  # hermes-fork
            if os.environ.get("VENICE_API_KEY", "").strip():
                return "venice"
            logger.warning(
    ...
    #     local > groq > openai > mistral > venice > xai > elevenlabs > deepinfra ---
    # hermes-fork: Venice stays a managed multi-modal fallback before xAI
    # (chain lives in _auto_detect_cloud_stt). DeepInfra is tried LAST per
    # upstream so DEEPINFRA_API_KEY never displaces an existing selection.
...
# ---------------------------------------------------------------------------
# Provider: Venice (/audio/transcriptions)  (hermes-fork)
# ---------------------------------------------------------------------------

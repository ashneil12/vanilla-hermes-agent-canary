...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_handles_tool_error(self, agent):
        ...
        assert len(messages) == 2
        # Concurrent scheduling does not guarantee which call reaches the
        # patched handler first, but the failed tool should be surfaced and the
        # other tool should still succeed.

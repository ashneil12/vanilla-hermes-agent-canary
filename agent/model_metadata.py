...
# hermes-fork: negative-cache timestamp — one failed probe suppresses further
# probes for _CODEX_OAUTH_CONTEXT_FAIL_TTL (startup-stall guard; see the
# backoff check in _fetch_codex_oauth_context_lengths_with_source).
...
def _fetch_codex_oauth_context_lengths_with_source(
    ...
    global _codex_oauth_context_fail_time  # hermes-fork: negative cache
    now = time.time()
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            _codex_oauth_context_fail_time = now  # hermes-fork: negative cache
            return {}, False
        data = resp.json()
    except Exception as exc:

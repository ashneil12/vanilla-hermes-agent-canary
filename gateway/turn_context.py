...
class TurnContext:
    ...
    # --- hermes-fork: runtime-governor billing meter -----------------------
    # DEFAULT-OFF. ``runtime_lease_id`` is None and ``_heartbeat_runtime_sync``
    # is None unless HERMES_RUNTIME_GOVERNOR_* is configured, so the extracted
    # TurnRunner methods behave exactly like upstream when the governor is off.
    runtime_lease_id: Optional[str] = None
    _heartbeat_runtime_sync: Optional[Callable[[], None]] = None

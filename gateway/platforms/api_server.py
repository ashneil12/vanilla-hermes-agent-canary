...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        toolsets_override: Optional[List[str]] = None,  # hermes-fork
    ...
    async def _run_agent(
        ...
        *requested_runtime* / *route_source* / *confirmed_runtime_lock*
        ...
        # hermes-fork + upstream composed: upstream's admitted-request
        # activation + in-flight concurrency accounting (_inflight_agent_runs)
        # wrap the fork's runtime-governor lease close (metered cutoff
        # surfacing). All must run on every path.
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        # NOTE: `route` is no longer resolved here — upstream hoisted
        # `route = self._resolve_route(body.get("model"))` (plus agent_overrides)
        # earlier in this function, so re-resolving would be a duplicate no-op.
            ...
            try:
                ...
                with self._profile_scope(request_profile):
                    agent = self._create_agent(
                        ...
                        gateway_session_key=gateway_session_key,
                        toolsets_override=toolsets_override,  # hermes-fork
                        requested_model=agent_overrides.get("requested_model"),
                        requested_provider=agent_overrides.get("requested_provider"),

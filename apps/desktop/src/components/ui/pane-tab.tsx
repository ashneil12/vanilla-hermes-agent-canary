...
export const PaneTab = React.forwardRef<HTMLDivElement, PaneTabProps>(function PaneTab(
  ...
  return (
    ...
    >
      {children}
      {/* hermes-fork: touch devices (webchat on phones) have neither middle-click
          nor ⌘-click, so the close gesture above is unreachable there. Show an
          explicit close target under (hover: none) only — hover-capable devices
          are unchanged. Mirrors canary "touch controls" work (fork PR #164). */}
      {onClose && (
        <button
          aria-label="Close tab"
          className={cn(
            'absolute hidden size-4 place-items-center rounded-sm text-(--ui-text-tertiary) active:bg-(--ui-bg-secondary) touch:grid',
      ...
      {dirty && (
        <span
          ...
          className={cn(
            ...
            vertical ? 'bottom-1.5 left-1/2 -translate-x-1/2' : 'right-1.5 top-1/2 -translate-y-1/2',
            // hermes-fork: the touch close button occupies this spot.
            onClose && 'touch:hidden'
          )}
        >
      ...
      {onClose && !vertical && (
        ...
        // VS Code-style.
        <span className="pointer-events-none absolute inset-y-0 right-0 flex items-stretch opacity-0 transition-opacity group-hover/tab:pointer-events-auto group-hover/tab:opacity-100 touch:hidden">

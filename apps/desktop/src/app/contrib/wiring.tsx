...
import { $narrowViewport, $newSessionTabAction, registerPaneCloser } from '@/components/pane-shell/tree/store'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // hermes-fork: dashboard iframe starter-prompt bridge (see hook docstring).
  useDashboardBridge({ submitText })

  // hermes-fork: narrow (phone) widths — any navigation (picking a session,
  // New Session, opening a route from the revealed sidebar) retires the pinned
  // pane reveals so the overlay never sits on top of the view the user just
  // navigated to. location.key changes on every navigate(), including
  // same-path ones. NarrowOverlays handles mode:'close' (a no-op when the
  // pane isn't revealed). Ported from the retired desktop-controller.tsx.
  useEffect(() => {
    if (typeof window === 'undefined' || !$narrowViewport.get()) {
      return
    }

    for (const id of ['sessions', 'files', 'review']) {

  ...
  return (
    ...
    >
      <SidebarRowShell
        actions={
          <div className="relative z-2 grid w-[1.375rem] place-items-center" data-row-actions>
            {!isWorking && (
              <span className="pointer-events-none absolute right-6 top-1/2 min-w-6 -translate-y-1/2 text-right text-[0.625rem] leading-none text-(--ui-text-tertiary) opacity-0 transition-opacity group-hover:opacity-100 touch:opacity-100">
                {age}
              </span>
            )}
            <SessionActionsMenu
      ...
      >
        ...
        <SidebarRowBody
          className={cn('z-0 group-hover:pr-12 touch:pr-12', branchStem && 'pl-3.5')}

    ...
    try:
        ...
        with _profile_scope(profile):
            return build_models_payload(
                load_picker_context(),
                # The settings model dropdown is search-filtered client-side, so
                # it needs the FULL per-provider catalog — not a 50-model
                # preview. Marketplace providers (Surplus Intelligence) expose
                # 200+ models; capping at 50 hid everything past index 50 from
                # the filter, so a model like `claude-opus-4-8-fast` (sorted into
                # the tail) was unreachable. 1000 covers every real provider

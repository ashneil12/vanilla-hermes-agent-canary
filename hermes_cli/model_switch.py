...
def provider_has_resolvable_credentials(
    ...
    *,
    ...
    found."*. The model-switch path calls this BEFORE persisting so the picker
    can't brick a box by selecting a full-catalog model under a provider the
    box has no key for (e.g. ``gpt-5.5-pro`` routing to ``openai-api`` while
    only ``SURPLUS_API_KEY`` is set).
